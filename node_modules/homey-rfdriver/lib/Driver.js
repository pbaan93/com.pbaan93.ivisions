...
class RFDriver extends Homey.Driver {
	...
	onPair(socket) { // Pair sequence
		...
		socket.on('program_send', (data, callback) => {
			...
			if (!pairDevice.assembleDeviceObject()) return callback(new Error('pairdevice_not_complete1'));
			const testPayload = pairDevice.constructor.dataToPayload(pairDevice.getData());
			if (testPayload && pairDevice.constructor.payloadToData(testPayload)) {
		...
		socket.on('test', (data, callback) => {
			...
			if (!pairDevice.assembleDeviceObject()) return callback(new Error('pairdevice_not_complete2'));
			callback(null, pairDevice.deviceState);
		});
		...
		socket.on('set_settings', (data, callback) => {
			...
			if (pairDevice) {
				pairDevice.setDeviceState({ settings: data });
		...
		socket.on('toggle', (capabilityId, callback) => {
			...
			if (!pairDevice) return callback(new Error('433_generator.error.no_device'));
			if (!pairDevice.assembleDeviceObject()) return callback(new Error('pairdevice_not_complete3'));
		...
		socket.on('capability', ({ id: capabilityId, value }, callback) => {
			...
			if (!pairDevice) return callback(new Error('433_generator.error.no_device'));
			if (!pairDevice.assembleDeviceObject()) return callback(new Error('pairdevice_not_complete4'));
			if (!pairDevice.hasCapability(capabilityId)) return callback(new Error(`pairdevice_does_not_have_capability_${capabilityId}`));
